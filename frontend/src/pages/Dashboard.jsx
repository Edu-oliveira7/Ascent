    ...
    <div className="flex h-screen bg-[#050505] text-[#f5f5f5] font-['Barlow'] overflow-hidden">
      ...
      <aside className="w-64 bg-[#0a0a0a] border-r border-white/5 flex flex-col p-6 hidden md:flex">
        ...
        <nav className="flex-1 space-y-2">
          ...
          <NavItem icon={<Dumbbell size={20} />} label="Meus Treinos" onClick={() => {}} />
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 border-b border-white/5 flex items-center justify-between px-8 bg-[#050505]/50 backdrop-blur-md">
          ...
          <div className="flex items-center gap-5">
            ...
            <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-[#ff301d] to-[#ff6b5c] border-2 border-white/10 overflow-hidden cursor-pointer">
               {/* Foto de perfil aqui futuramente */}
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
          ...
          <div className="flex items-end justify-between mb-10">
            <div>
              <h2 className="text-3xl font-semibold tracking-tight">Bem-vindo, Eduardo</h2>
              <p className="text-white/40 text-sm mt-1 font-light">Aqui está a visão geral da sua performance.</p>
            </div>
            <button 
              onClick={() => navigate("/create-workout")}
              className="bg-[#ff301d] hover:bg-[#d62818] text-white px-6 py-3 rounded-2xl font-bold text-sm transition-all shadow-lg shadow-[#ff301d]/20 flex items-center gap-2"
            >
              <Plus size={18} /> NOVO TREINO
            </button>
          </div>

          {/* STATS MINI CARDS (Igual a referência "My Portfolio") */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5 mb-10">
            <StatCard label="Treinos Semanal" value="04" subValue="+12%" />
            <StatCard label="Carga Total" value="1.240kg" subValue="+5.2%" />
            <StatCard label="Consistência" value="92%" subValue="Alta" />
            <StatCard label="Minutos" value="128m" subValue="-2%" />
          </div>
          ...
          <div className="bg-[#0a0a0a] border border-white/5 rounded-[2.5rem] p-8">
            ...
            {loading ? (
              <div className="flex justify-center py-20"><p className="animate-pulse text-white/20">Sincronizando dados...</p></div>
            ) : workouts.length === 0 ? (
              <div className="text-center py-20 bg-white/[0.01] rounded-3xl border border-dashed border-white/5">
                <p className="text-white/20">Sua jornada começa aqui. Crie seu primeiro treino.</p>
              </div>
            ) : (
...
function StatCard({ label, value, subValue }) {
  return (
    <div className="bg-[#111] border border-white/5 p-5 rounded-[2rem] hover:border-white/10 transition-colors shadow-xl">
